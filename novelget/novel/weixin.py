...
def web_init():
    ...
    # print(dic)
    for m in dic['ContactList']:
        print(m)
    return dic['User']['UserName'], dic['ContactList']
...
if __name__ == '__main__':
    ...
    myUserName, mlist = web_init()
    print('Login successfully %s can send messages now, input q to exit' % myUserName)
    msg = None
    while msg != 'q':
        if msg:
            for m in mlist:
                toUserName = m['UserName']
                send_msg(toUserName=toUserName)
        msg = input('>')
    print('Have fun:)')
